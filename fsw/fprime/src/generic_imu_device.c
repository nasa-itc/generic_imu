...
// Both of the below constants are defined to allow the greatest possible
// precision without overflow on a range of -10<x<10 for x=linear acceleration
// (in g) and -400<x<400 for x=angular rotation rate (in deg/s).
...
** Helper Function
*/
void GENERIC_IMU_FramePrep(can_info_t *device, uint8_t *data,
                           uint8_t data_len) {
  /* TX Frame */
...
int32_t GENERIC_IMU_ReadData(can_info_t *canDevice, uint8_t data_length) {
  ...
  if (status != CAN_SUCCESS) {
    ...
    OS_printf("GENERIC_IMU_ReadData: GENERIC_IMU_ReadData "
              "can_master_transaction failed with %d error! \n",
              status);
#endif
    status = CAN_ERROR;
  }
...
*/
int32_t GENERIC_IMU_RequestHK(can_info_t *canDevice,
                              GENERIC_IMU_Device_HK_tlm_t *data) {
  int32_t status = OS_SUCCESS;
...
** Request Axis
*/
int32_t GENERIC_IMU_RequestAxis(can_info_t *canDevice,
                                GENERIC_IMU_Device_Axis_Data_t *data,
                                uint8_t cmd_code) {
  int32_t status = OS_SUCCESS;
  uint32_t la_tmp;
  uint32_t aa_tmp;
  ...
  if (status == OS_SUCCESS) {
#ifdef GENERIC_IMU_CFG_DEBUG
    OS_printf("  GENERIC_IMU_RequestAxis %d, [%d] = ", cmd_code,
              canDevice->rx_frame.can_dlc);
    for (uint32_t i = 0; i < canDevice->rx_frame.can_dlc; i++) {
      OS_printf("%02x", canDevice->rx_frame.data[i]);
    }
    OS_printf("\n");
    ...
    /* Float conversion */
    data->LinearAcc =
        (float)((la_tmp - (LIN_CONV_CONST * 10.0)) / LIN_CONV_CONST);
    data->AngularAcc =
        (float)((aa_tmp - (ANG_CONV_CONST * 400.0)) / ANG_CONV_CONST);
  } else {
#ifdef GENERIC_IMU_CFG_DEBUG
...
*/
int32_t GENERIC_IMU_RequestData(can_info_t *canDevice,
                                GENERIC_IMU_Device_Data_tlm_t *data) {
  int32_t status = OS_SUCCESS;

  status = GENERIC_IMU_RequestAxis(canDevice, &data->X_Data,
                                   GENERIC_IMU_DEVICE_REQ_X_DATA_CMD);
  if (status == OS_SUCCESS) {
    status = GENERIC_IMU_RequestAxis(canDevice, &data->Y_Data,
                                     GENERIC_IMU_DEVICE_REQ_Y_DATA_CMD);
    if (status == OS_SUCCESS) {
      status = GENERIC_IMU_RequestAxis(canDevice, &data->Z_Data,
                                       GENERIC_IMU_DEVICE_REQ_Z_DATA_CMD);
    }
  }

#ifdef GENERIC_IMU_CFG_DEBUG
  if (status != OS_SUCCESS) {
    OS_printf("  GENERIC_IMU_RequestData: Error %d reported in "
